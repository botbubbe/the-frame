...
function ProspectsPage() {
  ...
  const [drawerData, setDrawerData] = useState<{
    ...
    contacts: { first_name: string; last_name: string; title: string; email: string; phone: string; is_primary: boolean }[];
  } | null>(null);
  ...
  const closeDrawer = () => {
    setDrawerOpen(false);
    setTimeout(() => { setDrawerProspectId(null); setDrawerData(null); }, 300);
  };

  // Close drawer on Escape
  useEffect(() => {
    const handler = (e: KeyboardEvent) => { if (e.key === "Escape" && drawerOpen) closeDrawer(); };
              ...
              {loading ? (
                ...
                <tr key={p.id} className={`group hover:bg-gray-50 dark:hover:bg-gray-700/50 cursor-pointer ${selected.has(p.id) ? "bg-blue-50/50 dark:bg-blue-900/10" : ""}`}
                  onClick={(e) => {
                    ...
                    // Cmd/Ctrl+click or middle-click → new tab (full detail page)
                    if (e.metaKey || e.ctrlKey || e.button === 1) {
                      const qs = navParams.toString();
                      window.open(`/prospects/${p.id}${qs ? `?${qs}` : ""}`, "_blank");
                    } else {
                      // Normal click → open side drawer
                      openDrawer(p.id);
                    }
                  }}
                  onMouseDown={(e) => {
